...
describe('forgot-password email helpers', () => {
  it('builds the Supabase Auth redirect URL from the current origin', () => {
    const url = buildRedirectTo('https://app.webisafe.ci/');

    expect(url).toBe('https://app.webisafe.ci/reinitialiser-mot-de-passe');
  });

  it('builds the email sent to customers with the reset link', () => {
    const email = buildPasswordResetEmail('https://supabase.example/auth/v1/verify?token=abc&type=recovery&redirect_to=https%3A%2F%2Fwebisafe.ci%2Freinitialiser-mot-de-passe');
    ...
    expect(email.html).toContain('https://supabase.example/auth/v1/verify?token=abc&type=recovery&redirect_to=https%3A%2F%2Fwebisafe.ci%2Freinitialiser-mot-de-passe');
