...
export default async function handler(req, res) {
  ...
  let body;
  try { body = await readJsonBody(req); } catch { return json(res, 400, { success: false, error: 'Corps invalide' }); }
  ...
  const missingField = ['payment_code', 'scan_id', 'user_email', 'url_to_audit', 'wave_phone'].find((field) => !payload[field]);
  ...
  try {
    ...
    console.error('[report-payment] email error:', error);
  }
