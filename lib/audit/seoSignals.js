...
export function analyzeSeoSignals($, url, probes = {}) {
  ...
  const aiChecks = [
    ...
    buildAiCheck('author_or_org', 'Auteur ou organisation identifiable', organizationTypes.length > 0 || brandCandidate.length >= 3, organizationTypes.join(', ') || brandCandidate || 'Signal absent.', 'Renforce l’attribution et la crédibilité des contenus.', 'Ajouter une organisation ou un auteur identifiable dans la page.'),
