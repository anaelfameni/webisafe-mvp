...
function qrCodeImage(url, size = 132, dataUri = null) {
  // dataUri : data URI pré-généré par qrcode (Node.js) — aucune dépendance réseau.
  // Si null (génération échouée), on omet le QR plutôt que d'inclure une image cassée.
  if (!dataUri) return '';
  return `<img class="qr-code" src="${h(dataUri)}" alt="QR code de vérification du rapport" width="${size}" height="${size}" />`;
}
...
function categoryScorePill(item) {
  ...
  const subtitleHtml = item.subtitle ? `<small class="score-pill-sub">${h(item.subtitle)}</small>` : '';
  return `<div class="score-pill"><span>${h(item.label)}</span><strong style="color:${color}">${value ?? '—'}</strong><em>${h(label)}</em>${subtitleHtml}</div>`;
}
...
function dataTable(columns, values, widths = [], limit = 7) {
  ...
  if (!visibleRows.length) return `<p class="empty">Aucun élément à signaler pour ce point.</p>`;
...
function coverSummaryBlock(model) {
  const s = model.coverSummary;
  if (!s) return '';
  const accentColor = s.topIssueSeverity === 'Critique' ? '#FCA5A5' : s.topIssueSeverity === 'À corriger' ? '#FDBA74' : '#94A3B8';
  const issueBlock = s.topIssue
    ? `<div class="cover-summary-issue"><span class="cover-summary-label" style="color:${accentColor}">Problème prioritaire</span><strong>${h(s.topIssue)}</strong>${s.topIssueAction ? `<p>${h(s.topIssueAction)}</p>` : ''}</div>`
    : `<div class="cover-summary-issue"><strong>Aucun problème critique détecté</strong></div>`;
  return `<div class="cover-summary"><div class="cover-summary-score"><span class="cover-summary-label">Score global</span><strong style="color:${h(s.globalColor)}">${h(s.globalScore)}</strong><em>${h(s.globalLabel)}</em></div><div class="cover-summary-sep"></div>${issueBlock}</div>`;
}
...
function coverPage(model) {
  ...
  const qrBlock = verifyUrl && model._qrDataUri ? `<div class="cover-verify"><div class="cover-verify-qr">${qrCodeImage(verifyUrl, 110, model._qrDataUri)}</div><div class="cover-verify-body"><span>Vérification du rapport</span><strong>Scannez ce QR code</strong><p>Accédez à la version en ligne de ce rapport et confirmez son authenticité.</p><em>${h(verifyUrl)}</em></div></div>` : '';
  return `<section class="cover premium-shell"><div class="cover-top">${brand(model)}<p>${h(model.report.confidentiality)}</p></div><div class="cover-grid"><div><p class="eyebrow">Audit cybersécurité exécutif</p><h1>Rapport d'audit ${h(brandName)}</h1><h2>${h(model.domain)}</h2><p class="lead">Analyse premium de la sécurité, de la performance, du SEO technique et de l'expérience mobile. Lecture orientée décision business, preuves observables et plan de correction.</p>${keyValueList(model.cover.metadata)}</div><aside class="cover-score"><span>Score global</span><strong style="color:${h(model.cover.scoreColor)}">${score ?? '—'}</strong><em>/100 · ${h(model.cover.scoreLabel)}</em><div>${badge(model.risk.label)}</div><p>${h(model.risk.summary)}</p></aside></div><div class="cover-scores">${model.cover.categoryScores.map(categoryScorePill).join('')}</div>${coverSummaryBlock(model)}${qrBlock}<div class="cover-footer"><p>${h(model.report.scanType)} · ID ${h(model.report.id)}</p><p>Page 1 / ${model.report.totalPages}</p></div></section>`;
}

function executivePage(model) {
  return page(model, 2, 'Verdict exécutif', 'Vue exécutive', 'Verdict exécutif', 'Synthèse décisionnelle des risques, de l\'impact business et du niveau de confiance du scan.', `<div class="executive-grid"><div class="verdict"><span>Verdict de notre équipe</span><h2>${h(model.executive.verdict)}</h2><p>${h(model.executive.businessSummary)}</p></div><div class="risk-level ${model.risk.tone}"><p>Niveau de risque</p><strong>${h(model.risk.label)}</strong><span>${h(model.executive.potentialImpact)}</span></div></div><div class="three-stats"><div><span>Confiance du scan</span><strong>${h(model.executive.scanConfidence.label)}</strong><p>${h(model.executive.scanConfidence.explanation)}</p></div><div><span>Critiques</span><strong>${model.counts.critical}</strong><p>Statuts critiques ou équivalents détectés.</p></div><div><span>À corriger</span><strong>${model.counts.warning}</strong><p>Points à planifier dans le prochain sprint.</p></div></div>${panel('Principales faiblesses', bulletList(model.executive.mainWeaknesses), 'accent')}<div class="two-cols">${panel('Décision recommandée', '<p class="text">Traiter les risques critiques en priorité, avant toute campagne de communication ou acquisition, puis relancer un scan pour mesurer la progression.</p>', 'warning')}${panel('Lecture business', '<p class="text">Ce rapport vous aide à prioriser les corrections urgentes, communiquer les enjeux à votre prestataire web et rassurer vos partenaires ou investisseurs.</p>', 'accent')}</div>`, '', 'executive');
}
...
function advancedSecurityPage(model) {
  ...
  return page(model, 8, 'Sécurité avancée', 'Contrôles étendus', 'Sécurité avancée', 'Risques moins visibles : protection anti-usurpation des emails, pare-feu applicatif, sous-domaines exposés et scripts tiers.', `${sectionScoreCard('Score sécurité avancée', section.score, section.status, section.diagnostic)}<div class="two-cols"><div>${panel('Contrôles principaux', dataTable(['Contrôle', 'Résultat', 'Statut'], section.summaryRows, ['32%', '48%', '20%']), 'accent')}</div><div>${panel('Sécurité email (anti-usurpation)', dataTable(['Élément', 'Résultat', 'Statut'], emailRows, ['30%', '50%', '20%']))}</div></div><div class="findings-grid slim">${criticalChecks.map((item) => technicalFinding(item.name, item.status, item.detail || item.name, item.key.includes('email') ? 'Des fraudeurs peuvent envoyer des emails en se faisant passer pour votre domaine, nuisant à votre réputation.' : 'Ce signal de sécurité avancée doit être corrigé selon la preuve détectée.', item.key.includes('security_txt') ? 'Créez un fichier /.well-known/security.txt indiquant un email de contact pour les signalements de sécurité.' : item.key.includes('waf') ? 'Activez un pare-feu applicatif (ex : Cloudflare gratuit) pour filtrer les requêtes malveillantes.' : 'Transmettez le détail ci-dessus à votre prestataire web pour correction.', 'Intermédiaire')).join('')}</div>`, 'compact', 'advancedSecurity');
}
...
function performancePage(model) {
  ...
  return page(model, 9, 'Performance', 'Core Web Vitals', 'Performance & conversion mobile', 'Vitesse perçue, stabilité, poids de page, origine serveur et limites de mesure.', `${sectionScoreCard('Score performance', section.score, section.status, section.diagnostic)}${metricGrid(section.metrics)}<div class="two-cols"><div>${panel('Localisation serveur', dataTable(['Signal', 'Valeur'], serverRows, ['35%', '65%']), 'accent')}</div><div>${panel('Optimisations prioritaires', dataTable(['Optimisation', 'Détail', 'Gain'], section.opportunities.map((item) => [item.title, item.description, item.savings]), ['34%', '48%', '18%']))}</div></div><div class="two-cols">${panel('Pourquoi la vitesse est cruciale', '<p class="text">La performance est mesurée sur mobile, car c\'est là que vos visiteurs vous jugent en premier. Chaque seconde de délai supplémentaire fait fuir une partie de vos visiteurs avant même qu\'ils lisent votre offre.</p>', 'accent')}${panel('Comment confirmer les améliorations', '<p class="text">Après les corrections de votre développeur, relancez un scan Webisafe. Comparez le score de performance et les indicateurs LCP, TBT et poids de page pour mesurer les progrès.</p>', 'warning')}</div>`, '', 'performance');
}

function seoUxPage(model) {
  const seo = model.sections.seo;
  const ux = model.sections.ux;
  return page(model, 10, 'SEO & UX', 'Visibilité et expérience', 'SEO technique & UX mobile', 'Compréhension par Google, partage social, accessibilité et obstacles à la conversion mobile.', `<div class="two-cols"><div>${scoreBar('Score SEO', seo.score, seo.diagnostic)}${metricGrid(seo.metrics)}${panel('Signaux complémentaires', dataTable(['Critère', 'Valeur', 'Statut'], [...seo.extraRows, ...seo.advancedRows], ['34%', '44%', '22%'], 16), 'accent')}</div><div>${scoreBar('Score UX Mobile', ux.score, ux.diagnostic)}${metricGrid([...ux.metrics, ux.tapTargets])}${panel('Problèmes UX détectés', dataTable(['Problème', 'Impact', 'Statut'], ux.issues.map((item) => [item.message, item.impact || item.type, item.severity]), ['42%', '40%', '18%']))}</div></div><div class="two-cols">${panel('Priorité SEO', '<p class="text">Commencez par corriger les éléments techniques manquants (titre, description, sitemap) avant de publier du nouveau contenu ou de lancer une campagne publicitaire. Sans ces bases, Google ne peut pas vous trouver correctement.</p>', 'accent')}${panel('Priorité UX mobile', '<p class="text">Corrigez en priorité les obstacles mobiles sur les pages les plus importantes : votre page d\'accueil, la page de contact et les pages produits ou services. Ce sont celles que vos visiteurs voient en premier.</p>', 'warning')}</div>`, 'compact', 'seo');
}
...
function styles() {
  return `<style>
    ...
    *{box-sizing:border-box}
    body{margin:0;background:#020617;font-family:Arial,sans-serif;color:${COLORS.text};-webkit-print-color-adjust:exact;print-color-adjust:exact}
    ...
    .kv-list{border-top:1px solid rgba(255,255,255,.12)}.kv-list p{display:flex;justify-content:space-between;gap:12px;margin:0;padding:9px 0;border-bottom:1px solid rgba(255,255,255,.10);font-size:9px}.kv-list span{color:#94A3B8;text-transform:uppercase;font-weight:800;letter-spacing:.5px}.kv-list strong{text-align:right;color:#F8FAFC}.score-pill,.metric,.panel,.risk-card,.action-card,.finding,.cta,.offer-grid article,.verdict,.risk-level,.three-stats>div,.score-bar,.scorecard-card{background:rgba(15,23,42,.82);border:1px solid rgba(148,163,184,.22);border-radius:16px;box-shadow:0 8px 22px rgba(0,0,0,.18)}.score-pill{padding:11px 10px}.score-pill span{display:block;color:#94A3B8;font-size:8px;text-transform:uppercase;font-weight:900}.score-pill strong{display:block;margin:6px 0 2px;font-size:24px}.score-pill em{font-size:8px;color:#CBD5E1;font-style:normal;text-transform:uppercase;font-weight:800}.score-pill-sub{display:block;font-size:7px;color:#64748B;font-style:normal;margin-top:3px;font-weight:600;letter-spacing:.3px}
...
export async function buildTemplate(scanData) {
  const model = buildPdfAuditModel(scanData);

  // Génération locale du QR code (élimine la dépendance à api.qrserver.com).
  // Si la génération échoue (qrcode absent ou erreur), le PDF est livré sans QR
  // plutôt que de bloquer la génération ou livrer une image cassée.
  const verifyUrlForQr = model.report?.verifyUrl || model.report?.brandUrl;
  if (verifyUrlForQr) {
    try {
      const QRCode = await import('qrcode');
      model._qrDataUri = await QRCode.default.toDataURL(verifyUrlForQr, {
        width: 110,
        margin: 1,
        color: { dark: '#1566F0', light: '#FFFFFF' },
        errorCorrectionLevel: 'M',
      });
    } catch (err) {
      console.warn('[PDF] QR code generation failed — PDF will render without QR:', err?.message || err);
      model._qrDataUri = null;
    }
  }
