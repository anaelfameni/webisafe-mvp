...
function humanizeCheckDetail(key, detail, data = {}) {
  ...
  // WAF : "Server=Vercel cf-ray=N/A" → reformulé lisiblement pour le client
  if (rawText(key).includes('waf')) {
    const detectedWaf = asArray(data?.detected);
    if (detectedWaf.length) return `Pare-feu détecté : ${detectedWaf.join(', ')}.`;
    // Extrait le nom du serveur si présent, sinon message générique
    const serverMatch = current.match(/server=([^\s]+)/i);
    const serverName = serverMatch ? serverMatch[1] : null;
    if (serverName && serverName !== 'N/A') return `Hébergement détecté via le serveur : ${serverName}. Aucun pare-feu applicatif identifié.`;
    return 'Aucune signature de pare-feu applicatif détectée dans les en-têtes HTTP.';
  }

  // CORS : masque les chaînes brutes de test ("Origin=*, credentials=false") derrière un texte clair
  if (rawText(key).includes('cors')) {
    const issueCount = asArray(data?.issues).length;
    if (issueCount) return `${issueCount} configuration(s) CORS permissive(s) détectée(s). Accès cross-origine non restreint.`;
    if (normalized === 'ok') return 'Politique CORS correctement configurée.';
    return current;
  }

  // Typosquatting : "Variantes actives : x, y, z" → on garde le contenu mais on nettoie le préfixe technique
  if (rawText(key).includes('typosquatting')) {
    const variantMatch = current.match(/variantes\s+actives\s*:\s*(.+)/i);
    if (variantMatch) return `Variantes de domaine enregistrées détectées : ${variantMatch[1]}.`;
    return current;
  }
...
function deduplicateDifficulty(raw) {
  if (!raw) return raw;
...
function normalizeRecommendation(item, index) {
  ...
  const rawDifficulty = pdfText(firstDefined(item?.difficulty, item?.difficulte, item?.difficulty_label, ''), '');
  return {
    title,
    ...
    difficulty: deduplicateDifficulty(rawDifficulty),
...
function generateExpertRecommendations(reportData) {
  ...
  extras.push({ priorite: 4, categorie: 'Performance', action: 'Alléger les scripts de la page (JavaScript)', explication: 'Demandez à votre développeur de réduire et différer le chargement des scripts non essentiels. Cela accélère affichage initial de la page.', impact: 'Site plus réactif au clic, moins abandons avant la prise de contact', difficulty: 'Technique', time: '2-4 heures' });
...
function buildPerformance(reportData, scores) {
  ...
  return {
    ...
    diagnostic: isPartial
      ? 'Le score de performance est partiel : certaines données sont indisponibles. Un nouveau scan est recommandé pour confirmer les priorités.'
      : 'La performance mesurée reflète la vitesse de chargement sur mobile, la stabilité visuelle et la facilité de navigation. Ces trois facteurs influencent directement votre taux de conversion.',
    metrics: [
...
function buildSecurity(reportData, scores) {
  ...
  return {
    ...
    metrics: [
      ...
      metric('Certificat SSL', firstDefined(sec.ssl_grade, sec.security_grade, 'Non vérifié'), sec.ssl_grade === 'A' || sec.ssl_grade === 'A+' ? 'OK' : (sec.ssl_grade && sec.ssl_grade !== 'Non vérifié') ? 'Avertissement' : 'Non mesuré', 'Qualité du chiffrement HTTPS. Un certificat défaillant affiche "Non sécurisé" dans Chrome.'),
      metric('HTTPS', yesNo(https, 'Activé', 'Non activé'), boolStatus(https), 'Chiffrement des données échangées entre le visiteur et votre site.'),
      metric('Réputation malware', sec.malware_detected === true ? 'Menace signalée' : sec.malware_detected === false ? 'Aucune menace détectée' : 'Non vérifié', sec.malware_detected === true ? 'Critique' : sec.malware_detected === false ? 'OK' : 'Non mesuré', 'Votre site est-il signalé comme dangereux par les antivirus en ligne ?'),
    ],
...
function buildAdvancedSecurity(reportData) {
  ...
  // Priorité : tech_cve (détecte Next.js via headers HTTP) > cms_detection du scan basique.
  // Évite la contradiction "Technologie personnalisée" dans la vue d'ensemble
  // alors que la sécurité avancée détecte correctement Next.js.
  const techCveData = checkData('tech_cve');
  const cmsDetection = firstDefined(
    techCveData?.cms_detection,
    sec.cms_detection,
    checkData('tech_and_dependencies')?.cms_detection,
    {}
  );
  ...
  return {
    score: baseScore,
    status,
    diagnostic: 'Les contrôles avancés détectent les risques moins visibles : usurpation de vos emails (SPF/DMARC), protection contre les attaques automatiques (WAF), sous-domaines exposés et dépendances tierces vulnérables.',
    summaryRows: [
...
function buildSeo(reportData, scores) {
  ...
  return {
    score: scores.seo,
    diagnostic: 'Le SEO technique mesure si Google peut trouver, lire et référencer votre site correctement. Un site mal configuré reste invisible, même avec du bon contenu.',
    metrics: [
...
function buildUx(reportData, scores) {
  ...
  return {
    score: scores.ux,
    diagnostic: 'L\'expérience mobile mesure les obstacles qui font fuir vos visiteurs avant le contact : textes trop petits, boutons difficiles à appuyer, zoom bloqué et temps de chargement.',
    metrics: [
...
function buildTopRisks(modelParts, recommendations, criticalAlerts) {
  ...
  sections.seo.extraRows.forEach((item) => {
    ...
    addRisk(risks, {
      ...
      recommendation: `Demandez à votre développeur d'ajouter ${item[0]} à votre site, puis relancez un scan Webisafe pour confirmer la correction.`,
      priority: 'P2',
      effort: 'Faible',
    });
  });
  sections.ux.issues.forEach((item) => {
    addRisk(risks, {
      ...
      recommendation: 'Demandez à votre développeur de corriger cet élément, puis testez votre site sur mobile en naviguant jusqu\'à la page de contact ou d\'achat.',
...
export function buildPdfAuditModel(reportData = {}) {
  ...
  // Bloc synthèse pour la cover : score global + problème le plus grave + action prioritaire
  const topCriticalRisk = topRisks.find(r => r.severity === 'Critique') || topRisks[0] || null;
  const coverSummary = {
    globalScore: scoreDisplay(scores.global),
    globalLabel: scoreLabel(scores.global),
    globalColor: scoreColor(scores.global),
    topIssue: topCriticalRisk ? pdfText(topCriticalRisk.title) : null,
    topIssueAction: topCriticalRisk ? pdfText(topCriticalRisk.recommendation) : null,
    topIssueSeverity: topCriticalRisk ? topCriticalRisk.severity : null,
  };
  ...
  return {
    ...
    report: {
      ...
      confidentiality: 'Document confidentiel - destiné au propriétaire du site audité',
      brandUrl,
      verifyUrl,
      // En marque blanche : on utilise le contact agence, pas le numéro Webisafe direct.
      contact: (() => {
        if (reportData.contact) return pdfText(reportData.contact);
        const branding = reportData.agencyBranding || reportData.agency_branding || {};
        if (branding.enabled === true && branding.contact_email) return pdfText(branding.contact_email);
        if (branding.enabled === true) return '';
        return 'WhatsApp +225 05 95 33 56 62';
      })(),
      totalPages: 13,
    },
    agencyBranding,
    ...
    cover: {
      ...
      metadata: compact([
        ...
        ['ID rapport', reportIdValue],
        reportData.scan_origin?.label ? ['Origine de mesure', pdfText(reportData.scan_origin.label)] : null,
      ]),
      categoryScores: compact([
        { label: 'Performance', score: scores.performance, status: severityFromScore(scores.performance) },
        { label: 'Sécurité', score: security.score, status: sectionStatus(security.score, security.missingHeaders.map((item) => item.severity)), subtitle: 'Headers & protections' },
        advancedSecurity.score !== null ? { label: 'Sécurité avancée', score: advancedSecurity.score, status: sectionStatus(advancedSecurity.score, advancedSecurity.checks.map((item) => item.status)), subtitle: 'Email & exposition' } : null,
