...
export function buildPdfAuditModel(reportData = {}) {
  ...
  const brandUrl = pdfText(firstDefined(reportData.brand_url, reportData.public_url, 'webisafe.app'));
  ...
  return {
    ...
    report: {
      id: reportId(domain, scanDate),
      scanType: 'Scan passif non intrusif',
      confidentiality: 'Confidentiel - usage interne',
      brandUrl,
      contact: pdfText(firstDefined(reportData.contact, 'WhatsApp +225 05 75 96 20 20')),
      totalPages: 11,
    },
    agencyBranding,
    risk,
    ...
    cover: {
      ...
      metadata: compact([
        ...
        ['Type de scan', 'Scan passif non intrusif'], ['ID rapport', reportId(domain, scanDate)],
        reportData.scan_origin?.label ? ['Origine de mesure', pdfText(reportData.scan_origin.label)] : null,
      ]),
