...
function buildSmartNarrative(norm) {
  ...
  const s = {
    ...
    sec: scores.security ?? 0,
    seo: scores.seo ?? 0,
    ux: scores.ux ?? 0,
  };

  // Ligne 1 : verdict global
  if (g >= 80) {
    lines.push("Votre site présente une excellente base technique. Quelques ajustements permettraient d'atteindre l'excellence.");
  } else if (g >= 60) {
    lines.push("Votre site est fonctionnel mais comporte des axes d'amélioration significatifs qui impactent son référencement et la confiance des visiteurs.");
  } else {
    lines.push("Votre site présente des problèmes techniques importants qui pénalisent fortement son référencement, sa sécurité et l'expérience utilisateur.");
  }

  // Ligne 2 : priorités concrètes (2-3 problèmes les plus graves)
  const problems = [];
  if (s.sec < 60) problems.push("la sécurité (HTTPS, headers, malware)");
  if (s.perf < 60) problems.push("la performance (temps de chargement, poids des pages)");
  if (s.seo < 60) problems.push("le SEO (titres, descriptions, indexation)");
  if (s.ux < 60) problems.push("l'expérience mobile (responsive, accessibilité)");

  if (problems.length > 0) {
    lines.push(`Priorités immédiates : ${problems.slice(0, 3).join(', ')}.`);
  } else if (g < 80) {
    lines.push("Les scores sont corrects mais quelques optimisations permettraient d'améliorer la conversion et le classement Google.");
  }
...
export default function Rapport() {
  ...
  const sections = [
    ...
    { id: 'extended_security', label: 'Sécurité Avancée', icon: <ShieldCheck size={16} /> },
  ...
  const premiumNarrative = buildSmartNarrative(norm);

  const scanDateRaw = norm?.scannedAt || scan.scanDate || scan.created_at || null;
  const scanDateText = formatScanDate(scanDateRaw) || formatDate(new Date().toISOString());
  const scanTimeText = formatScanTime(scanDateRaw);
      ...
      <div className="max-w-5xl mx-auto">
        ...
        {shareTooltip && (
          <motion.p
            initial={{ opacity: 0, y: -4 }}
            animate={{ opacity: 1, y: 0 }}
            className="text-success/90 text-xs mt-2 text-right"
          >
        ...
        <section id="recommendations" className="mb-12">
          ...
          <div className="space-y-4">
            ...
            {(() => {
              ...
              combined.sort((a, b) => (a.priorite ?? 4) - (b.priorite ?? 4));

              const urgent = combined.filter(r => (r.priorite ?? 4) <= 2);
              const medium = combined.filter(r => (r.priorite ?? 4) === 3);
              const improvements = combined.filter(r => (r.priorite ?? 4) >= 4);

              return (
                <>
                  {urgent.length > 0 && (
                    <div className="mb-4">
                      <p className="text-red-300 text-xs font-semibold uppercase tracking-wider mb-2"> Priorité urgente</p>
                      <div className="space-y-3">
                        {urgent.map((r, i) => (
                          <RecommendationCard key={`r-urgent-${i}`} recommendation={r} index={i} isLocked={false} />
                        ))}
                      </div>
                    </div>
                  )}
                  ...
                  {improvements.length > 0 && (
                    <div>
                      <p className="text-white/50 text-xs font-semibold uppercase tracking-wider mb-2"> Améliorations</p>
                      <div className="space-y-3">
                        {improvements.map((r, i) => (
                          <RecommendationCard key={`r-improvement-${i}`} recommendation={r} index={i} isLocked={false} />
                        ))}
                      </div>
                    </div>
                  )}
