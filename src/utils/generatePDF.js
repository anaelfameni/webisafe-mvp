...
function normalizeSecurity(scan, scores) {
  ...
  const extendedChecks = normalizeList(metrics.extended_checks ?? legacy.extended_checks);
  const extendedSecurityScore = metrics.extended_security_score ?? legacy.extended_security_score ?? null;
...
function drawPerformanceSecurity(doc, model, pageNum, totalPages) {
  ...
  // ── Sécurité Avancée (Extended Security) ──
  if (model.sections.security.extendedChecks.length > 0) {
    if (y > ph - 80) { y = addNewPage(doc, model, pageNum, totalPages, 'Securite Avancee'); y = sectionTitle(doc, 'Securite Avancee', y, margin); }
    else { y += 6; y = sectionTitle(doc, 'Securite Avancee', y, margin); }

    y = subtitle(doc, `Score securite avancee : ${model.sections.security.extendedSecurityScore ?? 'N/A'}/100`, margin, y);

    const extBody = model.sections.security.extendedChecks.map((check) => {
      const statusLabel = check.status === 'pass' ? 'OK' : check.status === 'fail' ? 'ECHEC' : 'AVERTISSEMENT';
      return [
        check.title || check.check_name || 'Controle',
        statusLabel,
        check.description || '',
        check.recommendation || '',
      ];
    });
    y = drawTable(doc, y, ['Controle', 'Statut', 'Description', 'Recommandation'], extBody, margin);
  }
}
