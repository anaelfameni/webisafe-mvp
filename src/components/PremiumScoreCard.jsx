...
export default function PremiumScoreCard({ score, ctaButton, onClose, compact = false, badgeLiftMobile = false }) {
  ...
  return (
    <div className={`relative w-full ${compact ? 'max-w-2xl' : 'max-w-4xl'} mx-auto mb-12`}>
      <div
        className={`bg-gradient-to-br from-[#0d1530] via-[#111827] to-[#0d1530] border border-white/5 rounded-[28px] ${compact ? 'p-6 sm:p-8' : 'p-8 sm:p-12'
      ...
      >
        ...
        {ctaButton && (
          <div className="mt-8 w-full flex justify-center z-10" onClick={onClose}>
            {ctaButton}
          </div>
        )}
      </div>
